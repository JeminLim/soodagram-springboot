...
public class UserBrowseController {	
	...
	public String accountGET(@PathVariable("userId") String userId, Model model, HttpServletRequest request) throws Exception {		
		...
		// 열람할 타겟 유저
		User target = (loginUser.getUserId().equals(userId)) ? loginUser : userService.getUserInfoById(userId);
		...
		if(loginUser.getUserId().equals(userId)) {
			List<User> recommendList = userService.getRecommendUserList(target);
